...
contract WinOnBuy is Ownable {
    ...
    event logSetBuyWinnersActive(bool buyWinnersActive);
    event logSetMinBuyToWin(uint256 oldMinBuyToWin, uint256 newMinimumBuyToWin);
    event logSetChanceToWinLastBuy(
        uint256 oldChanceToWin,
        uint256 newChanceToWin
    );
    event logSetChanceToWinSellTax(
        uint256 oldChanceToWin,
        uint256 newChanceToWin
    );
    event logSetChanceToWin0SellTax(
        uint256 oldChanceToWin,
        uint256 newChanceToWin
    );
    event logLastBuyWinner(address indexed winner, uint256 indexed amountWon);
    event logLastSellTaxWinner(
    ...
    function setMinimumBuyToWin(uint256 _minimumBuyToWin) external onlyOwner {
        ...
        minimumBuyToWin = _minimumBuyToWin * 10**18;
        emit logSetMinBuyToWin(_oldMinBuy, _minimumBuyToWin * 10**18);
    }
        ...
        onlyOwner
    {
        ...
        emit logSetChanceToWinSellTax(_oldChanceToWin, _chanceToWinSellTax);
    }
        ...
        onlyOwner
    {
        ...
        emit logSetChanceToWin0SellTax(_oldChanceToWin, _chanceToWin0SellTax);
    }
        ...
        returns (bool)
    {
        ...
        if (winningNumber == randomTxNumber) {
            return true;
        }
    }
